...
## ユーティリティー関数
- `void cnoid::calcMassMatrix(Body* body, const Vector3& g, MatrixXd& out_M)`: `6+n`x`6+n`の慣性行列を計算する. 事前に`void calcForwardKinematics()`と`calcCenterOfMass()`が必要.release-1.7にはバグがある. (`#include <cnoid/MassMatrix>`)
